...
namespace NovbatDehi.Class
{
    public class DbHelperReservations
    {
        ...
        public bool Add(Reservations reservation)
        {
            ...
            {
                _mydb.Open_Db();
                string sql = "INSERT INTO reservations(customer_id,_date,_time,paymentStatus,paymentKind,paymentCode,createTime,tozihat,FirstCome,_index,note)VALUES" +
                             "(@customer_id,@_date,@_time,@paymentStatus,@paymentKind,@paymentCode,@createTime,@tozihat,@FirstCome,@_index,@note)";
...
#pragma warning restore CS0168 // The variable 'exception' is declared but never used
        ...
        public Reservations Get_Reservation_By_id(int reservationId)
        {
            try
            {
                ...
                _mydb.Open_Db();
                SqlCommand cmd = new SqlCommand("select * from reservations where id= " + reservationId, _mydb.GetConnection_Db());
                cmd.CommandType = CommandType.Text;
        ...
        public List<Reservations> Get_All_Reservations(string _date = "")
        {
            try
            {
                _mydb.Open_Db();
                List<Reservations> locaList = new List<Reservations>();
                string sql =
                    "select * from reservations left join customers on reservations.customer_id=customers.id" +
                    (_date == "" ? "" : " where _date=N'" + _date + "'")
                    + " order by _time";
...
#pragma warning restore CS0168 // The variable 'exception' is declared but never used
        ...
        public bool Delete(int reservationsId)
        {
            ...
            {
                _mydb.Open_Db();
                SqlCommand cmd = new SqlCommand("delete from Reservations where id=" + reservationsId, _mydb.GetConnection_Db());
                cmd.CommandType = CommandType.Text;
                cmd.ExecuteNonQuery();
                _mydb.Close_Db();
                return true;
        ...
        public bool Update(int reservationsId, Reservations reservation)
        {
            try
            {
                ...
                string sql =
                    ...
                    ",paymentKind=@paymentKind,paymentCode=@paymentCode,createTime=@createTime,tozihat=@tozihat,FirstCome=@FirstCome,_index=@_index,note=@note  where id=@id";
                SqlCommand cmd = new SqlCommand(sql, _mydb.GetConnection_Db());
                cmd.CommandType = CommandType.Text;
                cmd.Parameters.AddWithValue("@FirstCome", reservation.FirstCome);
                cmd.Parameters.AddWithValue("@createTime", reservation.createTime);
